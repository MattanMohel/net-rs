...
const LEARN_RATE: f32 = 0.03;
const BATCH_SIZE: usize = 64;

struct Meta<const L: usize> {
    // layer sizes
    pub form: [usize; L],
...
struct Network<const L: usize> {
    ...
    // layer sums buffer
    sums: Vec<DMatrix<f32>>,
    // layer error buffer
    errors: Vec<DMatrix<f32>>,
    // layer error accumulation buffer
    error_acc: Vec<DMatrix<f32>>,
    // weight error buffer
    weight_errors: Vec<DMatrix<f32>>,
    // weight error accumulation buffer
    weight_error_acc: Vec<DMatrix<f32>>
}
...
impl<const L: usize> Network<L> {
    pub fn new(form: [usize; L]) -> Self {
        ...
        Self { 
            ...
            biases: {
                (1..form.len())
                    .map(|i| DMatrix::zeros(form[i], 1))
                    .collect()
            },
            
            weights: {
                (0..form.len()-1)
                    .map(|i| DMatrix::new(form[i+1], form[i]))
                    .collect()
            },
            ...
            weight_error_acc: {
                (0..form.len()-1)
                    .map(|i| DMatrix::zeros(form[i+1], form[i]))
                    .collect()
            }
        }
    }
